...
class CalendarWeekdayHeaderSection extends StatelessWidget {
  const CalendarWeekdayHeaderSection({super.key, required this.calendarProperties});

  final CalendarProperties calendarProperties;

  @override
  Widget build(BuildContext context) {
    List<String> weekdaysSymbolsList = getWeekdaysSymbolsListBasedOnStartingWeekday(
    ...
    return Container(
      decoration: BoxDecoration(
        color: calendarProperties.weekdaysProperties.backgroundColor,
        borderRadius: calendarProperties.weekdaysProperties.borderRadius,
      ),
...
class CalendarWeekdaySymbol extends StatelessWidget {
  const CalendarWeekdaySymbol({super.key, required this.symbol, required this.weekdayDecoration});
    ...
    return Center(
      child: Text(
        ...
        style: weekdayDecoration?.weekdayTextStyle
            ...
            const TextStyle().copyWith(color: weekdayDecoration?.weekdayTextColor),
        maxLines: 1,
